module ComputeResourcesVmsHelper
  def vm_power_actions(host, vm)
    button_group(
      if vm
        ...
        opts = hash_for_power_host_path(:power_action => vm.ready? ? :stop : :start).
          merge(:auth_object => host, :permission => 'power_hosts')
        link_to_if_authorized(
          icon_text('power-off', _("Power%s") % state(vm.ready?), :kind => 'fa'),
          opts,
  ...
  def vm_console(host, vm)
    if vm && vm.ready?
      link_to_if_authorized(
        icon_text('screen', _("Console"), :kind => 'pficon'),
        hash_for_console_host_path.merge(:auth_object => host,
                                         :permission => 'console_hosts'),
        :class => "btn btn-info")
    else
      link_to(icon_text('screen', _("Console"), :kind => 'pficon'),
              '#',
              :disabled=> true, :class => "btn btn-info")
    end
  end
